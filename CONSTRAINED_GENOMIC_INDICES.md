...
This document describes the implementation of constrained linear genomic
selection indices from Chapter 6 of Cerón-Rojas & Crossa (2018). All
mathematical formulas have been implemented in the new modular file
`constrained_genomic_indices.R`.

## Implementation Architecture

### Design Principles

1.  **Modular Structure**: All constrained genomic indices in a separate
    file for easy maintenance
2.  **C++ for Math**: All mathematical operations use primitives from
    `math_primitives.cpp`
3.  **R for Logic**: All selection index logic and algorithms
    implemented in R
4.  **Consistent API**: Functions follow the same pattern as existing
    package functions
...
**Key Implementation Details**: - Build augmented matrix using
[`rbind()`](https://rdrr.io/r/base/cbind.html),
[`cbind()`](https://rdrr.io/r/base/cbind.html) - Solve system using
[`MASS::ginv()`](https://rdrr.io/pkg/MASS/man/ginv.html) for
robustness - Extract β_RG from solution vector (first n_traits
elements) - Use
[`cpp_quadratic_form_sym()`](https://zankrut20.github.io/selection.index/reference/cpp_quadratic_form_sym.md)
for β’Γβ calculation - Response and gains computed in
[`.genomic_index_metrics()`](https://zankrut20.github.io/selection.index/reference/dot-genomic_index_metrics.md)
helper
...
**Key Implementation Details**: - First compute β_RG using internal call
to
[`rlgsi()`](https://zankrut20.github.io/selection.index/reference/rlgsi.md) -
Compute U’ΓU and invert using
[`solve()`](https://rdrr.io/r/base/solve.html) - Calculate θ_G using
matrix operations - Compute δ = U(U’ΓU)^(-1)d - Final coefficients: β_PG
= β_RG + θ_G \* δ - Verify proportionality by computing gain ratios
...
**Key Implementation Details**: - Build T_C matrix: \[P, P_yg; P_yg’,
P_g\] (2t x 2t) - Build Ψ_C matrix: \[G; C_gebv_g\] (2t x t) - Transform
constraint matrix: U_TC = Ψ_C %\*% U - Solve augmented system similar to
RLGSI - Split solution into b_y (phenotype) and b_g (GEBV)
coefficients - Use
[`.combined_index_metrics()`](https://zankrut20.github.io/selection.index/reference/dot-combined_index_metrics.md)
...
**Key Implementation Details**: - First compute β_CR using internal call
to
[`crlgsi()`](https://zankrut20.github.io/selection.index/reference/crlgsi.md) -
Compute Φ_C = Ψ_C %\*% U - Invert T_C using
[`MASS::ginv()`](https://rdrr.io/pkg/MASS/man/ginv.html) - Calculate
(Φ_C’ T_C^(-1) Φ_C)^(-1) - Compute unrestricted β_C = T_C^(-1) Ψ_C w -
Calculate θ_CP using formula - Compute δ_CP and final coefficients -
...
All mathematical operations leverage existing C++ functions from
`math_primitives.cpp`:

| Operation        | C++ Function                                                                                                  | Usage                               |
|------------------|---------------------------------------------------------------------------------------------------------------|-------------------------------------|
| Solve Ax = b     | [`cpp_symmetric_solve()`](https://zankrut20.github.io/selection.index/reference/cpp_symmetric_solve.md)       | Solving linear systems              |
| x’Ay             | [`cpp_quadratic_form()`](https://zankrut20.github.io/selection.index/reference/cpp_quadratic_form.md)         | Computing quadratic forms           |
| x’Ax             | [`cpp_quadratic_form_sym()`](https://zankrut20.github.io/selection.index/reference/cpp_quadratic_form_sym.md) | Computing symmetric quadratic forms |
| Extract elements | [`cpp_extract_vector()`](https://zankrut20.github.io/selection.index/reference/cpp_extract_vector.md)         | Extracting weight vectors           |

**No new C++ code was needed** - all existing primitives were
sufficient!
...
Computes standard metrics for genomic indices (RLGSI, PPG-LGSI): - Uses
Γ (GEBV variance-covariance) - Computes b’Γb, selection response,
expected gains - Returns accuracy (rHI), overall GA, PRE

### `.combined_index_metrics()`

Computes metrics for combined indices (CRLGSI, CPPG-LGSI): - Uses T_C
(combined variance) and Ψ_C (combined genetic covariance) - Computes
b’T_Cb, selection response, expected gains - Returns accuracy, GA, PRE
...
All functions return `constrained_response` to verify constraints are
satisfied: - RLGSI: U’Γβ ≈ 0 - PPG-LGSI: gain_ratios should be
constant - CRLGSI: U’E ≈ 0 - CPPG-LGSI: gain_ratios should be constant

### Numerical Stability

- All functions use
  [`MASS::ginv()`](https://rdrr.io/pkg/MASS/man/ginv.html) for matrix
  inversion (handles near-singular matrices)
...
Modern Plant Breeding*. Springer International Publishing. Chapter 6:
...
Combined Linear Genomic Selection Index
...
Predetermined Proportional Gains LPSI -
...
Predetermined Proportional Gains LGSI -
[`crlgsi()`](https://zankrut20.github.io/selection.index/reference/crlgsi.md) -
Combined Restricted LGSI -
[`cppg_lgsi()`](https://zankrut20.github.io/selection.index/reference/cppg_lgsi.md) -
Combined Predetermined Proportional Gains LGSI

### Consistent API Design

All functions share common parameter names: - `restricted_traits` -
User-friendly way to specify constraints - `U` - Advanced constraint
matrix option - `k_I` - Selection intensity - `GAY` - Comparative trait
for PRE calculation - `wmat`, `wcol` - Economic weights

All return lists with: - `summary` - Data frame with coefficients and
metrics - `b` - Coefficient vector - `E` - Expected genetic gains per
trait - `R` - Overall selection response
...
All extensions would follow the same principles: - C++ for computation -
R for logic - Modular organization - Consistent API

------------------------------------------------------------------------

## Conclusion

The implementation successfully translates all mathematical formulas
from Chapter 6 into efficient, well-documented R code. The modular
...
All code follows package conventions and uses existing C++ primitives,
requiring no new C++ development.
