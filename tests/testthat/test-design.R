test_that("rcbd.design calculates sums of products correctly", {
...
test_that("rcbd.design calculates mean products correctly", {
  gen_idx <- as.integer(as.factor(seldata$treat))
  rep_idx <- as.integer(as.factor(seldata$rep))
  trait1 <- seldata$sypp
...
test_that("rcbd.design returns all components with calc_type='all'", {
  gen_idx <- as.integer(as.factor(seldata$treat))
  rep_idx <- as.integer(as.factor(seldata$rep))
  trait1 <- seldata$sypp
  trait2 <- seldata$dtf
  
  result <- rcbd.design(trait1, trait2, gen_idx, rep_idx, calc_type = "all")
...
test_that("rcbd.design handles same trait (variance) correctly", {
  gen_idx <- as.integer(as.factor(seldata$treat))
  rep_idx <- as.integer(as.factor(seldata$rep))
  trait1 <- seldata$sypp
  
  # When trait1 = trait2, we're calculating variance components
  result <- rcbd.design(trait1, trait1, gen_idx, rep_idx, calc_type = "all")
...
test_that("rcbd.design calculates anova_stats correctly", {
  gen_idx <- as.integer(as.factor(seldata$treat))
  rep_idx <- as.integer(as.factor(seldata$rep))
  trait1 <- seldata$sypp
  
  result <- rcbd.design(trait1, trait1, gen_idx, rep_idx, calc_type = "anova_stats")
...
test_that("rcbd.design degrees of freedom are correct", {
  gen_idx <- as.integer(as.factor(seldata$treat))
  rep_idx <- as.integer(as.factor(seldata$rep))
  trait1 <- seldata$sypp
...
test_that("rcbd.design matches manual calculation", {
...
test_that("rcbd.design works with different trait combinations", {
  gen_idx <- as.integer(as.factor(seldata$treat))
  rep_idx <- as.integer(as.factor(seldata$rep))
...
test_that("rcbd.design covariance matches gen.varcov", {
  gen_idx <- as.integer(as.factor(seldata$treat))
  rep_idx <- as.integer(as.factor(seldata$rep))
  trait1 <- seldata$sypp
  trait2 <- seldata$dtf
  ...
  # Use design engine
  design_result <- rcbd.design(trait1, trait2, gen_idx, rep_idx, calc_type = "mean_products")
...
test_that("rcbd.design default calc_type is 'all'", {
  gen_idx <- as.integer(as.factor(seldata$treat))
  rep_idx <- as.integer(as.factor(seldata$rep))
  trait1 <- seldata$sypp
...
test_that("rcbd.design trait2 defaults to trait1", {
  gen_idx <- as.integer(as.factor(seldata$treat))
  rep_idx <- as.integer(as.factor(seldata$rep))
  trait1 <- seldata$sypp
  
  # Call without trait2 argument (should use trait1 for both)
  result1 <- rcbd.design(trait1, trait1, gen_idx, rep_idx, calc_type = "all")
  result2 <- rcbd.design(trait1, genotypes = gen_idx, replications = rep_idx, calc_type = "all")
