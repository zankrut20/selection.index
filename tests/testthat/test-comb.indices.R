...
test_that("comb.indices handles different ncomb values correctly", {
  ...
  cindex1<- comb.indices(ncomb = 1, pmat = pmat, gmat = gmat, wmat = weight[,-1], wcol = 1)
  expect_equal(nrow(cindex1), 7)  # choose(7, 1) = 7
  
  # Test pair combinations
  cindex2<- comb.indices(ncomb = 2, pmat = pmat, gmat = gmat, wmat = weight[,-1], wcol = 1)
...
test_that("comb.indices calculates PRE when GAY is provided", {
  gmat<- gen.varcov(seldata[,3:9], seldata[,2], seldata[,1])
  pmat<- phen.varcov(seldata[,3:9], seldata[,2], seldata[,1])
  cindex<- comb.indices(ncomb = 1, pmat = pmat, gmat = gmat, wmat = weight[,-1], wcol = 1, GAY = 1.075)
...
test_that("comb.indices works with different weight columns", {
  ...
  if(ncol(weight[,-1]) >= 2) {
    cindex2<- comb.indices(ncomb = 1, pmat = pmat, gmat = gmat, wmat = weight[,-1], wcol = 2)
