test_that("gen.advance calculates expected value", {
  ...
  weight<- matrix(1.00)
  GA<- round(gen.advance(phen_mat = pmat[1,1], gen_mat = gmat[1,1], weight_mat = weight),2)
...
test_that("gen.advance returns matrix output", {
  ...
  GA <- gen.advance(phen_mat = pmat[1,1], gen_mat = gmat[1,1], weight_mat = weight[1,2])
...
test_that("gen.advance works with variance-covariance matrices", {
  ...
  GA1 <- gen.advance(phen_mat = pmat[1,1], gen_mat = gmat[1,1], weight_mat = weight[1,2])
...
test_that("gen.advance handles matrix inputs correctly", {
  ...
  GA <- gen.advance(phen_mat = pmat, gen_mat = gmat, weight_mat = wmat)
  expect_true(is.matrix(GA))
