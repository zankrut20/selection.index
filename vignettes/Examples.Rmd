...
## New function - remove trait from the construction of selection indices
```{r}
rcomb.indices(ncomb = 1, i = 1, pmat = pmat, gmat = gmat, wmat = w[,-1], wcol = 1, GAY = GAY)
```

**Tips to use this package effectively**<br>
+ Must use new functions <br>
  - ```comb.indices``` - without removing any characters or trait<br>
  - ```rcomb.indices``` - by removing desired character <br>
+ Use for loop to make the selection indices construction <br>
+ Do not forget the Index of the trait/character when you calculating the selction score and ranking of genotypes.<br>
