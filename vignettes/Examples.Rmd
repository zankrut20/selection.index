...
## `comb_indices()` is used for construction of selection indices based on different combination of characters. 
```{r}
comb_indices(ncomb = 1, pmat = pmat, gmat = gmat, wmat = w[,-1], wcol = 1, GAY = GAY)
```

## `rcomb_indices()`` - remove trait from the construction of selection indices
```{r}
rcomb_indices(ncomb = 1, i = 1, pmat = pmat, gmat = gmat, wmat = w[,-1], wcol = 1, GAY = GAY)
```
